...
COUNT(id_filme) AS "# de filmes"  FROM filmes
GROUP BY genero
...
SELECT nota, COUNT(id_filme) AS "qtd de filmes" FROM alugueis
...
CREATE VIEW analise_nota_filme AS
SELECT nota, COUNT(id_filme) AS "qtd de filmes" FROM alugueis
WHERE nota >= (SELECT AVG(nota) FROM alugueis)
GROUP BY nota
ORDER BY nota;

SELECT * FROM analise_nota_filme;